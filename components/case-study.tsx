...
export function CaseStudy({ slug }: { slug: string }) {
  ...
  const name = isLokal ? 'Lokal' : isInfluencers ? 'Influencers' : isGrails ? 'Our Grails' : slug === 'field-notes' ? 'Field Notes' : slug === 'forma' ? 'Forma' : slug === 'quiet-hours' ? 'Quiet Hours' : 'Atlas'
  const company = isLokal || isInfluencers ? 'eBay' : isGrails ? 'University of Washington' : 'Independent case study'
  const sections = isLokal ? lokalSections : isInfluencers ? influencerSections : isGrails ? grailsSections : defaultSections
  const metadata = isLokal ? { role: 'Product Designer', deliverables: 'Web & Native UI, UX', timeline: '6 months' } : isInfluencers ? { role: 'Product Designer', deliverables: 'Web & Native UI, UX', timeline: '4 months' } : isGrails ? { role: 'Researcher, Designer', deliverables: 'Thesis, Publication, Gallery Exhibit', timeline: '12 months' } : { role: 'Design direction, Product design', deliverables: 'Strategy, UX, Visual system', timeline: '12 weeks · 2024' }
  ...
  return (
    <div className={dark ? 'site dark' : 'site'}>
      ...
      <main className="case-study">
        ...
        <div className="case-images"><div className="placeholder portrait">{isLokal ? 'Lokal portrait image · /public/lokal/portrait-1.png' : isGrails ? 'Our Grails portrait image placeholder' : 'Portrait image placeholder'}</div><div className="placeholder portrait">{isLokal ? 'Lokal portrait image · /public/lokal/portrait-2.png' : isGrails ? 'Our Grails portrait image placeholder' : 'Portrait image placeholder'}</div></div>
