  ...
  return (
    <div className={dark ? 'site dark' : 'site'}>
      ...
      <main className="case-study">
        ...
        {isGrails ? <div className="grails-gallery">{grailsImages.map((src, index) => <Fragment key={src}>{index === 0 && <section className="grails-text-block"><h2>{sections[0].title}</h2><p className="section-subtitle">{sections[0].subtitle}</p><p>{sections[0].body}</p></section>} {index === 6 && <section className="grails-text-block"><h2>{sections[1].title}</h2><p className="section-subtitle">{sections[1].subtitle}</p><p>{sections[1].body}</p></section>} {index === 12 && <section className="grails-text-block"><h2>{sections[2].title}</h2><p className="section-subtitle">{sections[2].subtitle}</p><p>{sections[2].body}</p></section>}<img className="placeholder landscape case-hero-image" src={src} alt={`Our Grails body ${index + 1}`} /></Fragment>)}<div className="case-images">{grailsPortraits.map((src, index) => <img className="placeholder portrait case-portrait-image" key={src} src={src} alt={`Our Grails body 17 side ${index === 0 ? 'A' : 'B'}`} />)}</div></div> : landscapeImage ? <img className="placeholder landscape case-hero-image" src={landscapeImage} alt={landscapeAlt} /> : <div className="placeholder landscape">Landscape image placeholder</div>}
        {!isGrails && <div className="case-copy">{sections.map((section) => <section key={section.title}><h2>{section.title}</h2><p className="section-subtitle">{section.subtitle}</p><p>{section.body}</p></section>)}</div>}
