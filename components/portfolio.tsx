...
export function Portfolio() {
  ...
  return <div className={dark ? 'site dark' : 'site'}>
    ...
    <main id="top"><section id="projects" className="projects" aria-labelledby="projects-title"><div className="section-heading"><h2 id="projects-title">Selected work</h2><span>01—04</span></div><div className="project-grid">{projects.map((project, index) => <article className="project-card" key={project.title}><Link href={`/case-studies/${project.slug}`} className={`project-image ${project.color}`} aria-label={`View ${project.title} project`}><Image src={project.image} alt={`${project.title} project`} fill sizes="(max-width: 1279px) 100vw, 50vw" priority={index < 2} /></Link><div className="project-meta"><h3>{project.title}</h3><div><span>{project.type}</span><span className="meta-dot">·</span><span>{project.year}</span></div></div></article>)}</div></section><section id="about" className="about-section"><div className="section-heading"><h2>About</h2><span>01—02</span></div><div className="about-grid"><p className="about-lede">Designing digital products, identities, and experiences for teams building what&apos;s next.</p><div className="about-detail"><p>Over the past decade I&apos;ve worked across product, brand, and strategy — from early-stage startups to global teams. I like clear questions, good constraints, and the moment a complicated idea clicks.</p><a href="mailto:hello@julianbody.com">Get in touch <span>↗</span></a></div></div></section></main>
